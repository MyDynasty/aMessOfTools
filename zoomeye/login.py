...
def resource_info(access_token):
    ...
    """
    authorization = {'Authorization': 'JWT ' + access_token["access_token"]}
    return requests.get("https://api.zoomeye.org/resources-info", headers=authorization).content
...
def mian():
    ...
    result = list()
    if search:
        max_page = int(input('请输入最大页数(每页20条):\n'))
        while page <= max_page:
            ...
            page += 1
    result = set(result)
    with open(mode + '_' + query.replace(':','').replace('"','') + '.txt', "w") as f:     #without /\:*"?<>|
        f.writelines(result)
    f.close()
    print(resource_info(access_token))
